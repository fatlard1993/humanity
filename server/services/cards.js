...
const Cards = {
	packs: {},
	init: function(){
		browse(`${__dirname}/../cards`, function(data){
	...
	recordCustom: function(text){
		...
		fs.readFile(`${__dirname}/../cards/custom.json`, function(err, data){
			var packData;
			...
			fs.writeFile(`${__dirname}/../cards/custom.json`, JSON.stringify(packData), function(err){
				Log()(`Saved custom`);
			});
		});
	}
