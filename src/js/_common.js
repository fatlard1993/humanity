var Cjs = {
	clone: function clone(it){
		if(typeof it !== 'object') return console.error('common', 'Cjs.clone only accepts Objects and Arrays');

		if(it instanceof Array) return it.slice(0);

		// console.warn('common', 'Cjs.clone uses JSON stringify => parse to clone an object, some things may not behave as expected: Dates, functions, Infinity, undefined, etc..');
	...
	decToHex: function decToHex(dec, length){
		...
		if(output === 'NaN'){
			...
			console.error('dec2hex isNaN', dec, length, output);
		}
