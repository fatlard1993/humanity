	...
	constructor(rootFolder, port) {
		...
		this.sockets.registerEndpoints({
			...
			player_register: function (payload) {
				...
				if (game.rooms[roomID] && game.rooms[roomID].players[name] && game.rooms[roomID].players[name].state !== 'inactive')
					return this.reply('player_register', { error: 'A player by that name is already playing' });
